...
export function MiniGameCard({ config, role, onDismiss, onResult }: Props) {
  ...
  return (
    <div className="relative">
      ...
      {finished && (
        <div
          className="absolute inset-0 z-10 flex flex-col justify-end cursor-pointer rounded-xl transition-opacity duration-300 bg-black/10"
          onClick={onDismiss}
          role="button"
          tabIndex={0}
        ...
        >
          ...
          >
            <span className="text-sm text-white/70">Tap to continue</span>
          </div>
          <style>{`
