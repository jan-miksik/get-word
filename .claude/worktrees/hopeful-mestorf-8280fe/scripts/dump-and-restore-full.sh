...
# Extract hostname from DATABASE_URL for validation
...
if [ -n "$SUPABASE_CONTAINER" ]; then
  ...
  # Extract connection details from LOCAL_DB_URL for docker exec
...
if ! psql "$REMOTE_DB_URL" -c "SELECT 1;" > /dev/null 2>&1; then
  ...
  echo -e "${YELLOW}   1. IPv6 Resolution Required:${NC}"
  echo -e "${YELLOW}      Supabase Direct Connections require IPv6. If DNS fails, you may need:${NC}"
  echo -e "${YELLOW}      - Enable IPv6 on your system, OR${NC}"
  echo -e "${YELLOW}      - Enable the IPv4 add-on in Supabase Dashboard${NC}"
  echo -e "${YELLOW}      - Settings → Database → Connection Pooling → IPv4 add-on${NC}"
  echo -e "${YELLOW}   2. Alternative: Use TypeScript migration script instead:${NC}"
  echo -e "${YELLOW}      pnpm run db:migrate-to-supabase${NC}"
  echo -e "${YELLOW}      (Uses Drizzle ORM which works with pooler connections)${NC}"
  echo -e "${YELLOW}   3. Other checks:${NC}"
  echo -e "${YELLOW}      - Internet connection${NC}"
  echo -e "${YELLOW}      - DATABASE_URL in .env.local is correct${NC}"
  echo -e "${YELLOW}      - Remote Supabase project is active (not paused)${NC}"
  echo -e "${YELLOW}   The dump file ${DUMP_FILE} is available for manual restore${NC}"
  exit 1
fi
