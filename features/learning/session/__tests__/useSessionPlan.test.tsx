...
const goal = {
  id: 'goal-1', effectiveFromDay: scope.dayKey, enabled: true, daysPerWeek: 4,
  minutesPerDay: 10, wordsPerDay: 5, preset: 'medium' as const,
...
describe('useSessionPlan persistence', () => {
  ...
  it('does not freeze a plan before session data is ready, then persists it once ready', async () => {
    ...
    rerender({ ready: true });
    await waitFor(() => expect(readSessionPlan(scope)?.blocks).toHaveLength(2));
    expect(result.current.dailyPlan?.blocks).toHaveLength(2);
  });
  ...
  it('keeps the frozen plan available while overflow uses the unbounded stream', async () => {
    ...
    await waitFor(() => expect(result.current.dailyPlan?.blocks).toHaveLength(2));
  ...
  it('recovers a completed frozen plan after reload when the live stream is empty', async () => {
    ...
    await waitFor(() => expect(second.result.current.dailyPlan?.blocks).toHaveLength(2));
  });
});
