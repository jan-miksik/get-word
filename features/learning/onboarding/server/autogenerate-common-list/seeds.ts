...
export async function findSeedCandidates(options: { excludeListIds?: string[] } = {}) {
  ...
  const rows = await db
    ...
    // Someone's personal word-chat list is never seed material for the shared
    // common list, even when it is public.
    .where(and(eq(wordLists.isPublic, true), eq(wordLists.isPersonal, false)))
...
export async function loadSeedDetails(seed: Pick<WordList, "id">) {
