...
export function LearningLanguageOnboarding({
  ...
  // editor): subscribe to the recommended exact-direction list when one exists,
  // otherwise autogenerate a common list (including flipping a reverse-direction
  // recommendation into the requested direction). Public-but-not-recommended
  // exact matches are deliberately NOT auto-selected here — they remain available
  // for manual pick under advanced options, but Continue only ever commits to a
  // vetted recommended list or a freshly autogenerated one. A seed-only
  // recommendation (`fallback_seed`) is likewise not forked — forking opens the
