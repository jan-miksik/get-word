...
export function LearningLanguageOnboarding({
  ...
  // picks a sensible default: a recommended/matching list when one exists, a fork
  // of the seed list as a fallback, or an autogenerated common list otherwise.
  function handleContinue() {
    if (!canContinue || loadingMatches || workingId !== null) return;
    if (recommendedReason === 'fallback_seed' && recommendedList) {
      void forkList(recommendedList);
    } else if (recommendedList) {
        ...
        <div className="onboarding-divider mt-6 pt-5">
          {!advancedOpen ? (
            <div className="space-y-3">
              <button
                type="button"
                className="onboarding-option onboarding-option-highlight w-full rounded-xl px-5 py-4 text-center text-lg font-extrabold shadow-sm disabled:cursor-not-allowed disabled:opacity-50"
