...
/** True once the feature tour has been finished or skipped; it never repeats. */
export function readFeatureTourAnswered(): boolean {
  if (typeof window === 'undefined') return false;
  return localStorage.getItem(FEATURE_TOUR_ANSWERED_KEY) === '1';
}

export function persistFeatureTourAnswered(answered: boolean): void {
  if (typeof window === 'undefined') return;
  if (answered) {
