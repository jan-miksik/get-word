...
import { usePendingListAudioMarker } from '@/features/lists/hooks/usePendingListAudioMarker';
import { useListsMaintenanceActions } from '@/features/lists/hooks/useListsMaintenanceActions';
...
import type { WizardActiveStep, WordListItem } from '@/features/lists/types';
import { ErrorMessage } from '@/features/lists/components/ErrorMessage';
import { ListSidebar } from '@/features/lists/components/list-sidebar/ListSidebar';
import { CategoryBrowser } from '@/features/lists/components/category-browser/CategoryBrowser';
import { TextareaEditor } from '@/features/lists/components/TextareaEditor';
import { DiffPreview } from '@/features/lists/components/DiffPreview';
import { TranslationStep } from '@/features/lists/components/translation-step/TranslationStep';
...
function ListsPageContent() {
  ...
  const pendingAudioListId = usePendingListAudioMarker({ selectedListId, items });
  const markedLists = useMemo(
    () => lists.map((list) => ({ ...list, audioIncomplete: list.id === pendingAudioListId })),
    [lists, pendingAudioListId],
  );
  const markedSelectedList = selectedList
    ? { ...selectedList, audioIncomplete: selectedList.id === pendingAudioListId }
    : null;
  ...
  const {
    assignItemCategory: handleAssignItemCategory,
    previewAutogeneratedRepair: handlePreviewAutogeneratedListRepair,
    repairAutogenerated: handleRepairAutogeneratedList,
  } = useListsMaintenanceActions({
    selectedListId,
    applyUpdatedList,
    reloadListDetails,
    setError,
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border-subtle bg-background-elevated">
          ...
          <Link
            ...
            aria-label={t('moderation.myReportsTitle')}
            title={t('moderation.myReportsTitle')}
          >
        ...
        <div className="flex-1 overflow-y-auto overscroll-y-contain">
          ...
          {!markedSelectedList ? (
            ...
            <CategoryBrowser
              ...
              onFork={startFork}
              onDeleteList={deleteList}
              onContentHidden={hideListFromView}
              onListUpdated={applyUpdatedList}
              onItemsChanged={reloadListDetails}
              onPreviewAutogeneratedRepair={handlePreviewAutogeneratedListRepair}
              onRepairAutogenerated={handleRepairAutogeneratedList}
            />
