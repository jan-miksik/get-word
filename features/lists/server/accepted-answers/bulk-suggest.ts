...
interface AcceptedAnswerList {
  id: string;
  languageFrom: string;
  languageTo: string;
}

export class AcceptedAnswerSuggestionError extends Error {
  constructor(message: string, readonly status: number) {
    super(message);
    this.name = "AcceptedAnswerSuggestionError";
  }
}
...
export async function suggestAcceptedAnswersBulk(input: {
  userId: string;
  list: AcceptedAnswerList;
  body: Record<string, unknown>;
}) {
  const { userId, list, body } = input;
  const listId = list.id;
  const rawItemIds = Array.isArray(body.item_ids) ? body.item_ids : null;
  if (!rawItemIds || rawItemIds.length === 0) {
    throw new AcceptedAnswerSuggestionError("item_ids array is required", 400);
  }
  ...
  if (itemIds.length > BULK_ACCEPTED_ANSWERS_CHUNK_SIZE) {
    throw new AcceptedAnswerSuggestionError(
      `item_ids is limited to ${BULK_ACCEPTED_ANSWERS_CHUNK_SIZE} items per request`,
      400,
    );
  }
  ...
  if (promptItems.length === 0) {
    return { suggestions: [], skipped_item_ids: skippedItemIds };
  }
  ...
  try {
    ...
    throw new AcceptedAnswerSuggestionError(message, 502);
  }
