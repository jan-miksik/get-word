...
interface AcceptedAnswerSuggestionList {
  id: string;
  languageFrom: string;
  languageTo: string;
}

export class AcceptedAnswerItemSuggestionError extends Error {
  constructor(message: string, readonly status: number) {
    super(message);
    this.name = "AcceptedAnswerItemSuggestionError";
  }
}
...
export async function suggestAcceptedAnswersForItem(input: {
  userId: string;
  list: AcceptedAnswerSuggestionList;
  itemId: string;
  body: Record<string, unknown>;
}) {
  const { userId, list, itemId, body } = input;
  const listId = list.id;
  const side = body.side === "known" ? "known" : body.side === "target" ? "target" : null;
  if (!side) {
    throw new AcceptedAnswerItemSuggestionError("side must be 'known' or 'target'", 400);
  }
  ...
  if (!item || item.listId !== listId) {
    throw new AcceptedAnswerItemSuggestionError("Item not found", 404);
  }

  const apiKey = await getUserApiKey(userId, "openrouter");
  if (!apiKey) {
    throw new AcceptedAnswerItemSuggestionError(
