...
import { updateItemTranslations } from "@/lib/db";
...
export class AcceptedAnswerApplyError extends Error {
  readonly status = 400;

  constructor(message: string) {
    super(message);
    this.name = "AcceptedAnswerApplyError";
  }
}
...
export async function applyAcceptedAnswersBulk(input: {
  listId: string;
  ...
  if (!entries || entries.length === 0) {
    throw new AcceptedAnswerApplyError("items array is required");
  }
  ...
  if (updates.length > 0) {
    try {
      ...
      if (error instanceof AcceptedAnswersValidationError) {
        throw new AcceptedAnswerApplyError(error.message);
      }
      throw error;
    }
  }
