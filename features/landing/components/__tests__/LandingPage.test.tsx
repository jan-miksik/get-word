...
describe('LandingPage language selector', () => {
  ...
  it('uses the shared interface selector with only bundled UI languages', () => {
    render(<LandingPage />);
    ...
    expect(
      screen.getByRole('button', { name: /Request another app language/i }),
    ).toBeInTheDocument();
  });

  it('switches landing copy when selecting an existing bundled language', async () => {
    render(<LandingPage />);

    fireEvent.click(screen.getByRole('button', { name: 'App language' }));
    fireEvent.click(screen.getByRole('option', { name: /Czech/i }));

    expect(
      await screen.findByRole('heading', { name: 'Slovíčka, která ti zůstanou' }),
    ).toBeInTheDocument();
  ...
  it('falls back to bundled English for an obsolete unsupported saved locale', async () => {
    localStorage.setItem('get-word-landing-lang', 'es');

    render(<LandingPage />);

    expect(await screen.findByRole('heading', { name: 'Words that stay with you' })).toBeInTheDocument();
    expect(
      vi.mocked(fetch).mock.calls.some(([input]) => String(input).startsWith('/api/i18n/messages')),
    ).toBe(false);
  });
});
