...
- ONE meaning per item. Never bundle synonyms or near-synonyms into a single item with a comma, "or", "and", a dash, or a semicolon: "nevýrazný, fádní" and "slaný, pikantní" are two items each, not one, and "slaný" and "pikantní" are not even the same meaning. A comma is allowed only inside a genuine sentence that needs one — never as a separator between alternative words. Pick the single best wording; if a second one is also worth teaching, spend a separate item slot on it or drop it.
