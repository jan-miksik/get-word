...
- Do not answer a single-word or short-phrase source with a list of alternatives. One source wording gets one target wording: no "nevýrazný, fádní", no "salty, spicy", no comma-, "or"-, dash- or semicolon-joined synonyms. The only commas in a target are the ones a real sentence needs.
...
- ONE meaning per item. Never bundle synonyms or near-synonyms into a single row with a comma, "or", "and", a dash, or a semicolon: "bland, flavourless" and "chewy, tough" are two rows each, not one. A comma belongs only inside a real sentence that needs one, never as a separator between alternative words. Each side of a row is a single wording the learner can read, type, or say as it stands.
