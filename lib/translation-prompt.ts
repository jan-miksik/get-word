...
- Treat each list as connected teaching material, not isolated jobs. When several items share the same beginner meaning or function, reuse the same natural target wording (a shared "teaching anchor") so the learner recognizes the pattern across the list and can connect related words and example sentences.
- Anchor consistency follows meaning, not surface form. Keep the same target wording only when the source word has the same meaning/function. When a repeated source word plays a different role, or when a natural target genuinely needs a different construction, use the natural construction instead of forcing the anchor — never reuse a target just because the source word looks the same.
...
- MOST ITEMS SHOULD HAVE NO COMMENT. Return a comment ONLY when it adds clear learning value, e.g.:
  ...
  - a teaching-anchor mismatch: the natural translation does not visibly use the target wording a learner would expect from a related item in the same list, especially in beginner lists. Add a short, practical note when a learner might expect the same source word to map to the same target word, but the target language uses a different construction here. Add these only when the mismatch is likely to confuse a beginner — not for every variation — and explain the construction in a phrase-based way without criticizing the translation,
