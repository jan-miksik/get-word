...
// Get the canonical app editor (oldest editor account). Used to own
// autogenerated common lists so they are app-owned, not learner-owned.
export async function getAnyEditorUserId(): Promise<string | null> {
  const results = await db
    .select({ id: users.id })
    .from(users)
